...
import { vLocalize } from '../../../directives';
...
const archetype = ref<GameItem<ArchetypeDataModel> | undefined>();
const career = ref<GameItem<CareerDataModel> | undefined>();
...
function updateArchetypeAndCareer() {
	...
	archetype.value = actor.items.find((i) => i.type === 'archetype') as GameItem<ArchetypeDataModel> | undefined;
	career.value = actor.items.find((i) => i.type === 'career') as GameItem<CareerDataModel> | undefined;
}

async function openItemSheet(item?: GameItem) {
	if (!item) {
		return;
	}
...
async function removeArchetype(archetype?: GameItem<ArchetypeDataModel>) {
	if (!archetype) {
		return;
	}
...
async function removeCareer(career?: GameItem<CareerDataModel>) {
	if (!career) {
		return;
	}
...
<template>
	<header class="character-meta">
		<div class="data">
			...
			<label><Localized label="EoA.Labels.CharacterName" /></label>
			<input type="text" name="name" :value="rootContext.data.actor.name" v-localize:placeholder="'EoA.Labels.CharacterName'" />

			<div class="underline"></div>

			<label><Localized label="EoA.Labels.SpeciesArchetype" /></label>
			<div>
				...
				<span v-else><Localized label="EoA.Labels.NoArchetype" /></span>
			</div>

			<div class="underline"></div>

			<label><Localized label="EoA.Labels.Career" /></label>
			<div>
				...
				<span v-else><Localized label="EoA.Labels.NoCareer" /></span>
			</div>
