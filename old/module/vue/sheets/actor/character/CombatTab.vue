...
const weapons = computed(() => actor.value.items.filter((i) => i.type === 'weapon' && i.system.state === 'equipped') as GameItem<WeaponDataModel>[]);
const armors = computed(() => actor.value.items.filter((i) => i.type === 'armor' && i.system.state === 'equipped') as GameItem<ArmorDataModel>[]);
...
async function rollAttack(weapon: GameItem<WeaponDataModel>) {
...
async function healInjury(injury: GameItem<InjuryDataModel>) {
...
function skillForWeapon(weapon: GameItem<WeaponDataModel>): GameItem<SkillDataModel> | undefined {
	const weaponSkills = weapon.systemData.skills.map((s) => s.toLowerCase());

	return toRaw(rootContext.data.actor).items.find((i) => i.type === 'skill' && weaponSkills.includes(i.name.toLowerCase())) as GameItem<SkillDataModel> | undefined;
}

function damageForWeapon(weapon: GameItem<WeaponDataModel>) {
...
<template>
	<section class="tab-combat">
		<div class="block">
			<div class="header"><Localized label="EoA.Labels.Attacks" /></div>
			<div class="weapons">
				<div class="weapon header">
					<div class="name"><Localized label="EoA.Labels.Name" /></div>
					<div class="skill"><Localized label="EoA.Labels.Skill" /></div>
					<div class="damage"><Localized label="EoA.Labels.Damage" /></div>
					<div class="crit"><Localized label="EoA.Labels.Crit" /></div>
					<div class="range"><Localized label="EoA.Labels.Range" /></div>
				</div>
				...
				<div v-for="weapon in weapons" :key="weapon.id" class="weapon">
					...
					<div class="skill" v-for="skill in [skillForWeapon(weapon)]" :key="skill?.id ?? 'skill'">
						...
						<span v-else><Localized label="EoA.Labels.SkillNotFound" /></span>
					</div>
					<div class="damage">{{ damageForWeapon(weapon) }}</div>
					<div class="crit">{{ weapon.systemData.critical }}</div>
					<div class="range"><Localized :label="`EoA.Range.${weapon.systemData.range.capitalize()}`" /></div>
		...
		<div class="block">
			<div class="header"><Localized label="EoA.Labels.Armor" /></div>
			<div class="armors">
				<div class="armor header">
					<div class="name"><Localized label="EoA.Labels.Name" /></div>
					<div class="soak"><Localized label="EoA.Labels.Soak" /></div>
					<div class="defense"><Localized label="EoA.Labels.Defense" /></div>
				</div>
		...
		<div class="block">
			<div class="header"><Localized label="EoA.Labels.CriticalInjuries" /></div>
			<div class="critical-injuries">
				<div class="injury header">
					<div><Localized label="EoA.Labels.Severity" /></div>
					<div><Localized label="EoA.Labels.Injury" /></div>
				</div>
...
.tab-combat {
	...
	.block {
		...
		.critical-injuries {
			...
			.severity {
				font-family: 'EoA Symbols', sans-serif;
				font-size: 1.25em;
