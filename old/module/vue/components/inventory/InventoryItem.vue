...
const props = withDefaults(
	defineProps<{
		item: GameItem<EquipmentDataModel>;
		dragging?: boolean;
		mini?: boolean;
	}>(),
	{
...
const emit = defineEmits<{
	...
	(e: 'equipmentStateChange', desiredState: EquipmentState, items: GameItem[]): void;
}>();
...
const containedItems = computed(() => toRaw(rootContext.data.actor).items.filter((i) => (i as GameItem<EquipmentDataModel>).systemData.container === props.item.id) as GameItem<EquipmentDataModel>[]);
	...
	if (props.item.type === 'container') {
		...
		await Promise.all(
			toRaw(rootContext.data.actor)
				.items.filter((i) => (i as GameItem<EquipmentDataModel>).systemData.container === itemId)
				.map((i) => i.delete()),
		);
	}
		...
		props.item.type !== 'weapon'
			...
			: {
					...
					damage:
						...
						((system.value as WeaponDataModel).damageCharacteristic !== '-' ? ' + ' + game.i18n.localize(`EoA.CharacteristicAbbr.${(system.value as WeaponDataModel).damageCharacteristic.capitalize()}`) : ''),
			  };

	const chatTemplate = await renderTemplate('systems/eoa/templates/chat/item.hbs', {
		img: props.item.img,
...
async function changeItemQuantity(item: GameItem, value: number) {
	if (value <= 0) {
...
async function changeDamageState(item: GameItem, currentState: EquipmentDamageState, direction: number) {
...
async function drop(event: DragEvent) {
	...
	emit('equipmentStateChange', props.item.systemData.state, [item as GameItem]);
...
<template>
	...
	>
			...
			<div :data-item-type="item.type">
				...
				<template v-if="item.type === 'weapon'">
					...
					<div>
						...
						<span v-else-if="weaponData.baseDamage >= 0"> <Localized :label="`EoA.CharacteristicAbbr.${weaponData.damageCharacteristic.capitalize()}`" />+{{ weaponData.baseDamage }} ({{ weaponDamage }})</span>
						<span v-else><Localized :label="`EoA.CharacteristicAbbr.${weaponData.damageCharacteristic.capitalize()}`" />{{ weaponData.baseDamage }} ({{ weaponDamage }})</span>
					</div>
					...
					<!-- Range -->
					<div>Range [<Localized :label="`EoA.Range.${weaponData.range.capitalize()}`" />]</div>
		...
		<a class="quantity" @click="changeItemQuantity(item, item.systemData.quantity + 1)" @contextmenu="changeItemQuantity(item, item.systemData.quantity - 1)" v-if="item.type !== 'container'" data-tooltip="EoA.Labels.Quantity">
			{{ item.systemData.quantity }}
			<i class="fas fa-backpack"></i>
		</a>

		<div class="weight" data-tooltip="EoA.Labels.Encumbrance">{{ system.encumbrance < 0 ? '+' : null }}{{ Math.abs(system.encumbrance) }} <i class="fas fa-weight-hanging"></i></div>

		<a
			class="damage"
			@click="changeDamageState(item, item.systemData.damage, +1)"
			@contextmenu="changeDamageState(item, item.systemData.damage, -1)"
			:data-tooltip="'EoA.Equipment.DamageState.' + item.systemData.damage.capitalize()"
		>
