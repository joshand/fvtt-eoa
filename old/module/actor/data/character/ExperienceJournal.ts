...
export async function removeJournalEntry(actor: GameActor<CharacterDataModel>, index: number) {
	...
	switch (removedEntry.type) {
		case EntryType.Characteristic: {
			...
			if (
				...
				ui.notifications.info(game.i18n.format('EoA.Notifications.CannotDeleteCharacteristic', removedEntry.data));
				return;
			}
		...
		case EntryType.Skill: {
			...
			const skill = <GameItem<SkillDataModel> | undefined>actor.items.find((i) => i.type === 'skill' && i.id === data.id);
			if (!skill) {
				break;
			}
			...
			if (actor.systemData.experienceJournal.entries.find((e, i) => i !== index && e.type === EntryType.Skill && (<SkillEntryData>e.data).id === data.id && (<SkillEntryData>e.data).rank > data.rank)) {
				ui.notifications.info(game.i18n.format('EoA.Notifications.CannotDeleteSkill', removedEntry.data));
				return;
			}
		...
		case EntryType.NewTalent: {
			...
			const talent = <GameItem<TalentDataModel> | undefined>actor.items.find((i) => i.type === 'talent' && i.id === data.id);
			if (!talent) {
				break;
			}

			if (data.rank !== talent.system.rank) {
				ui.notifications.info(game.i18n.format('EoA.Notifications.CannotDeleteExistingTalent', data));
				return;
			}
