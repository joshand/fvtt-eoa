...
import { EntryType, ExperienceJournal } from './character/ExperienceJournal';
...
type CharacterActor = GameActor<CharacterDataModel>;
type ArmorItem = GameItem<ArmorDataModel>;
type EquipmentItem = GameItem<EquipmentDataModel>;

export default abstract class CharacterDataModel extends foundry.abstract.DataModel implements IHasPreCreate<GameActor<CharacterDataModel>> {
	...
	async preCreate(actor: GameActor<this>, _data: PreDocumentId<any>, _options: DocumentModificationContext<GameActor<CharacterDataModel>>, _user: User) {
		...
		if (skillsCompendiumName.length === 0) {
			ui.notifications.warn(game.i18n.localize('EoA.Notifications.NoSkillsCompendium'));
		...
		if (!pack) {
			ui.notifications.error(game.i18n.format('EoA.Notifications.MissingCompendium', { name: skillsCompendiumName }));
			return;
		}
		if (pack.metadata.type !== 'Item') {
			ui.notifications.error(game.i18n.format('EoA.Notifications.WrongCompendiumType', { name: skillsCompendiumName, type: pack.metadata.type }));
			return;
		}
		const skillIds = pack.index.filter((i) => i.type === 'skill');
		if (skillIds.length === 0) {
			ui.notifications.warn(game.i18n.format('EoA.Notifications.NoSkillsInCompendium', { name: skillsCompendiumName }));
			return;
		}

		// Populate the skill items
		const skillItems = (await pack.getDocuments()).filter((i) => (<GameItem>i).type === 'skill').map((i) => i.toObject());
