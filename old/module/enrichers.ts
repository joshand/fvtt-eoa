...
export function register() {
	CONFIG.TextEditor.enrichers = [
		...
		/**
		 * Enricher to easily add EoA dice into the document.
		 *
		 ...
		 */
		{
			...
			enricher: async (match, _) => {
				...
				container.className = 'font-eoa-symbols';
		...
		/**
		 * Enricher to easily add EoA symbols into the document.
		 */
		{
			pattern: /@sym(bols?)?\[(?<symbols>\w+)]/gim,
			enricher: async (match, _) => {
				const span = document.createElement('span');
				span.className = `font-eoa-symbols nolig ${CONFIG.eoa.useMagicalGirlSymbols ? 'mg' : ''}`;
		 ...
		 */
		{
			...
			enricher: async (match, _) => {
				...
				container.innerHTML =
					...
					game.i18n.format('EoA.Enrichers.Difficulty', {
