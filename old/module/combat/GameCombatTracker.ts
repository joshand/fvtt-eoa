...
import GameCombat, { InitiativeSkill } from './GameCombat';
...
export default class GameCombatTracker extends CombatTracker<GameCombat> {
	override get template(): string {
		return 'systems/eoa/templates/sidebar/combat-tracker.hbs';
	}
	...
	async initiativeSkills() {
		if (!this.#initiativeSkills || this.#initiativeSkills.length === 0) {
			...
			this.#initiativeSkills = (await compendium.getDocuments())
				.filter((i) => (i as Item).type === 'skill' && (i as GameItem<SkillDataModel>).systemData.initiative)
				.map((s) => ({ skillName: s.name, skillChar: (s as GameItem<SkillDataModel>).systemData.characteristic }));
		}
	...
	async _onClaimInitiativeSlot(event: JQuery.ClickEvent) {
		...
		const combatant = userToken.combatant as GameCombatant;
		...
		if (
			(combatant.disposition === 'friendly' && (this.viewed.turns[slotIndex] as GameCombatant).disposition !== 'friendly') ||
			(combatant.disposition !== 'friendly' && (this.viewed.turns[slotIndex] as GameCombatant).disposition === 'friendly')
		) {
			ui.notifications.warn(game.i18n.format('Game.Notifications.CannotClaimOppositeSlot', { name: combatant.name }));
			return;
		}
	...
	override async getData(options: CombatTrackerOptions) {
		...
		const turns = data.turns.map((t, index) => {
			const combatant = combat.combatants.get(t.id) as GameCombatant;
			const claimantId = combat.claimantForSlot(combat.round, index);
			const claimant = claimantId ? (combat.combatants.get(claimantId) as GameCombatant) : undefined;
	...
	protected override _getEntryContextOptions(): EntryContextOption[] {
		...
		const revokeClaim = {
			name: 'Game.CombatTracker.RevokeInitiativeClaim',
	...
	protected override async _onCombatantControl(event: JQuery.ClickEvent<HTMLElement, HTMLElement, HTMLElement>) {
		...
		switch (btn.dataset.control) {
			...
			case 'cycleInitiativeSkill':
				const combatant = this.viewed.combatants.get(li.dataset.combatantId as string) as GameCombatant | undefined;
