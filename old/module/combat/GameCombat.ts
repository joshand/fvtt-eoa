...
type CombatSlotInfo = {
	slotOrigin: GameCombatant;
	initiative: number | null;
	disposition: 'friendly' | 'neutral' | 'hostile';
	id: string;
};

export type GameRollInitiativeOptions = {
	prompt?: boolean;
...
export default class GameCombat extends Combat {
	initiativeSkills: InitiativeSkill[] = [];

	claimantForSlot(round: number, slot: number): string | undefined {
		const claimants = this.getFlag('eoa', FLAG_CLAIMANTS) as ClaimantData | undefined;
	...
	async claimSlot(round: number, slot: number, combatantId: string) {
		...
		await this.setFlag('eoa', FLAG_CLAIMANTS, claimants);
	}
	...
	async revokeSlot(round: number, slot: number) {
		...
		await this.unsetFlag('eoa', `claimants.${round}.${slot}`);
	}

	extraSlotsForRound(round: number) {
		const extraSlots = (this.getFlag('eoa', FLAG_EXTRA_SLOTS) ?? []) as Omit<ExtraCombatSlot, 'index'>[];
	...
	async addExtraSlot(combatantId: string, round: number) {
		...
		await this.setFlag('eoa', FLAG_EXTRA_SLOTS, extraSlots);
	}
	...
	override async rollInitiative(ids: string | string[], { formula = null, updateTurn = true, messageOptions = {} }: RollInitiativeOptions = {}, { prompt = false, extraSlotsRolls = [] }: GameRollInitiativeOptions = {}) {
		...
		for (const [i, id] of ids.entries()) {
			// Combatant data
			const combatant = this.combatants.get(id) as Embedded<GameCombatant> | undefined;
			...
			const rollData = {
				description: game.i18n.format('Game.Rolls.Description.Initiative', { skill: skillName }),
				results,
			};
	...
	async addInitiativeSlot() {
		...
		if (controlledTokenCount !== 1) {
			ui.notifications.warn(game.i18n.localize('Game.Notifications.SelectOneTokenForAction'));
			return;
		}

		const combatant = canvas.tokens.controlled[0].combatant as GameCombatant;
		if (!combatant) {
			ui.notifications.warn(game.i18n.localize('Game.Notifications.TokenIsNotCombatant'));
			return;
		}
	...
	override async rollAll(options?: RollInitiativeOptions | undefined, onlyNPCs: boolean = false) {
		...
		const extraSlotsRolls = this.extraSlotsForRound(this.round).reduce((accum, slot) => {
			if (slot.initiative === null) {
				const combatant = this.combatants.get(slot.activationSource) as GameCombatant | undefined;
	...
	override setupTurns() {
		const combatants = this.combatants.map(
			(combatant) =>
				({
					slotOrigin: combatant as GameCombatant,
					initiative: combatant.initiative,
					disposition: (combatant as GameCombatant).disposition,
					id: combatant.id,
				} as CombatSlotInfo),
		);
...
/**
 * Register socket listener for Game Combats
 */
export function register() {
	...
	game.socket.on(SOCKET_NAME, async (payload: SocketPayload<CombatSocketBaseData>) => {
		...
		const combat = game.combats.get(payload.data.combatId) as GameCombat | undefined;
		if (!combat) {
