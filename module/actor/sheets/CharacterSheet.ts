 ...
 */
export default class CharacterSheet extends VueSheet(GameActorSheet<CharacterDataModel>) {
	...
	protected override async _onDropItem(event: DragEvent, data: DropCanvasData<'Item', GameItem<BaseItemDataModel>>): Promise<GameItem<BaseItemDataModel>[] | boolean> {
		...
		if (droppedItem.type === 'archetype') {
			...
			const existingArchetype = <GameItem<ArchetypeDataModel> | undefined>this.actor.items.find((i) => i.type === 'archetype');
			if (existingArchetype) {
				await this.removeArchetype(existingArchetype);
			}

			await this.applyArchetype(<GameItem<ArchetypeDataModel>>droppedItem);
		}
		...
		if (droppedItem.type === 'career') {
			...
			const existingCareer = <GameItem<CareerDataModel> | undefined>this.actor.items.find((i) => i.type === 'career');
			if (existingCareer) {
				await this.removeCareer(existingCareer);
			}

			await this.applyCareer(<GameItem<CareerDataModel>>droppedItem);

			return false; // applyCareer handles adding the embedded document to the character.
		}

		if (droppedItem.type === 'talent') {
			const talent = <GameItem<TalentDataModel>>droppedItem;
			const existingTalent = <GameItem<TalentDataModel>>this.actor.items.find((i) => i.type === 'talent' && i.name.toLowerCase() === talent.name.toLowerCase());

			if (existingTalent) {
				if (existingTalent.systemData.ranked === 'no') {
					ui.notifications.info(game.i18n.format('EoA.Notifications.TalentNotRanked', { talentName: existingTalent.name }));
					return false;
				}
				...
				if (this.actor.systemData.availableXP < cost) {
					ui.notifications.info(game.i18n.format('EoA.Notifications.CannotAffordRankedTalent', { name: talent.name, newRank, cost }));
					return false;
				}
				...
				if (this.actor.systemData.availableXP < cost) {
					ui.notifications.info(game.i18n.format('EoA.Notifications.CannotAffordTalent', { name: talent.name, cost }));
					return false;
				}

				const [newTalent] = (await this._onDropItemCreate(talent.toObject())) as GameItem<TalentDataModel>[];
		...
		if (['weapon', 'armor'].includes(droppedItem.type)) {
			...
			await Promise.all(
				this.actor.effects
					.filter((effect) => (effect as GameEffect).originItem?.id === equipable.id && !effect.disabled)
					.map(
						async (effect) =>
							await effect.update({
								disabled: true,
	 ...
	 * @param archetype
	 */
	async applyArchetype(archetype: GameItem<ArchetypeDataModel>) {
	 ...
	 * @param archetype
	 */
	async removeArchetype(archetype: GameItem<ArchetypeDataModel>) {
		const workingData = <CharacterDataModel>deepClone(this.actor.systemData);
		const archetypeData = archetype.systemData;
	 ...
	 * @param droppedCareer
	 */
	async applyCareer(droppedCareer: GameItem<CareerDataModel>) {
		...
		const commonSkills = <GameItem<SkillDataModel>[]>this.actor.items.filter((i) => i.type === 'skill' && careerSkillNames.includes(i.name.toLowerCase()));
	 ...
	 * @param career
	 */
	async removeCareer(career: GameItem<CareerDataModel>) {
		const careerData = career.systemData;

		// Unmark Career Skills
		const careerSkillNames = careerData.careerSkills.map((s) => s.name);
		const skills = <GameItem<SkillDataModel>[]>this.actor.items.filter((i) => i.type === 'skill');
