...
import { BaseSheetContext, RootContext } from '../SheetContext';
...
type EffectSection = {
	label: string;
	filter: (effect: GameEffect) => boolean;
};
...
async function openEffect(effect: GameEffect) {
	await toRaw(effect).sheet.render(true);
}

async function suppressEffect(effect: GameEffect) {
	await toRaw(effect).update({
		disabled: !effect.disabled,
	});
}

async function deleteEffect(effect: GameEffect) {
	...
	<section class="effects-view">
		<template v-for="{ label: section, filter } in sections" :key="section">
			<div class="effects-header">
				<div class="name"><Localized :label="`EoA.ActiveEffects.${section}`" /></div>
				<div class="source"><Localized label="EoA.ActiveEffects.Source" /></div>
				<div class="duration"><Localized label="EoA.ActiveEffects.Duration" /></div>
				<div class="buttons">
					<a @click="emit('addEffect', section.toLowerCase())"><i class="fas fa-plus"></i> <Localized label="EoA.Labels.Add" /></a>
				</div>
			</div>
