...
import XPContainer from '../../../components/character/XPContainer.vue';
...
const skills = computed(() => toRaw(context.data.actor).items.filter((i) => i.type === 'skill') as GameItem<SkillDataModel>[]);
...
const markCareerLabel = game.i18n.localize('EoA.Labels.MarkCareerSkill');
const unmarkCareerLabel = game.i18n.localize('EoA.Labels.UnmarkCareerSkill');
const freeRankUpLabel = game.i18n.localize('EoA.Labels.FreeRankUp');
const freeRankDownLabel = game.i18n.localize('EoA.Labels.FreeRankDown');
...
async function purchaseSkillRank(skill: GameItem<SkillDataModel>) {
...
async function toggleCareerSkill(skill: GameItem<SkillDataModel>) {
	await toRaw(skill).update({
		'system.career': !skill.systemData.career,
	});
}

async function freeSkillRank(skill: GameItem<SkillDataModel>, adjustment: number) {
	await toRaw(skill).update({
...
async function editSkill(skill: GameItem<SkillDataModel>) {
	await toRaw(skill).sheet?.render(true);
}
async function deleteSkill(skill: GameItem<SkillDataModel>) {
...
<template>
	<section class="tab-skills">
			...
			<MasonryWall :column-width="300" :items="skillCategories" :gap="8">
				<template #default="{ item: skillCategory, index }">
					<div class="skill-category" :style="`position: relative; z-index: ${skillCategories.length - index}`">
						<div class="header">
							<label><Localized :label="`EoA.Labels.${skillCategory.capitalize()}Skills`" /></label>
							<div class="blank" />

							<label style="position: relative; left: -3px"><Localized label="EoA.Labels.Rank" /></label>
							<div class="blank" />
						</div>

						<div class="body">
							<ContextMenu
								v-for="skill in skills.filter(s => s.systemData.category === skillCategory).sort((l: GameItem, r: GameItem) => (l.name < r.name) ? -1 : (l.name > r.name) ? 1 : 0)"
								:key="skill.id"
								class="skill row"
							>
								...
								<a class="name" @click="rollSkill(skill)">
									<span>{{ skill.name }} (<Localized :label="`EoA.CharacteristicAbbr.${skill.system.characteristic.capitalize()}`" />)</span>
		...
		<section class="experience">
			<XPContainer label="EoA.Labels.TotalXP" :value="system.totalXP" />
			<XPContainer label="EoA.Labels.AvailableXP" :value="system.availableXP" />
		</section>
	</section>
</template>
