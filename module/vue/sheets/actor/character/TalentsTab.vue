...
const activeAbilities = computed(() => toRaw(context.data.actor).items.filter((i) => i.type === 'ability' && (i.system as AbilityDataModel).activation.type === 'active') as GameItem<AbilityDataModel>[]);
...
const passiveAbilities = computed(() => toRaw(context.data.actor).items.filter((i) => i.type === 'ability' && (i.system as AbilityDataModel).activation.type === 'passive') as GameItem<AbilityDataModel>[]);
const archetypeAbilities = computed(() => (toRaw(context.data.actor).items.find((i) => i.type === 'archetype') as GameItem<ArchetypeDataModel>)?.systemData.grantedItems.filter((g) => g.type === 'ability').map((r) => r.name) ?? []);

const allTalents = computed(() => toRaw(context.data.actor).items.filter((i) => i.type === 'talent') as GameItem<TalentDataModel>[]);
...
async function upgradeTalent(talent: GameItem<TalentDataModel>) {
	if (talent.systemData.ranked === 'no') {
		ui.notifications.info(game.i18n.format('EoA.Notifications.TalentNotRanked', { talentName: talent.name }));
		return false;
	}
...
async function openItem(item: GameItem) {
...
<template>
	<section class="tab-talents">
		<div class="block">
			<div class="header"><Localized label="EoA.Labels.SpecialAbilities" /></div>
			<div class="talents-container">
				<!-- Active Abilities -->
				<div v-if="activeAbilities.length > 0" class="category-header"><Localized label="EoA.Labels.Active" /></div>
				<Talent
				...
				<!-- Passive Abilities -->
				<div v-if="passiveAbilities.length > 0" class="category-header"><Localized label="EoA.Labels.Passive" /></div>
				<Talent
			...
			<div class="header"><Localized label="EoA.Labels.Talents" /></div>
			<div class="talents-container">
				<!-- Active Talents -->
				<div v-if="activeTalents.length > 0" class="category-header"><Localized label="EoA.Labels.Active" /></div>
				<Talent
				...
				<!-- Passive Talents -->
				<div v-if="passiveTalents.length > 0" class="category-header"><Localized label="EoA.Labels.Passive" /></div>
				<Talent
		...
		<div class="pyramid">
			<div><Localized label="EoA.Labels.TierCount" :format-args="{ tier: 1 }" />: {{ tier1Talents }}/∞</div>
			<div><Localized label="EoA.Labels.TierCount" :format-args="{ tier: 2 }" />: {{ tier2Talents }}/{{ Math.max(0, tier1Talents - 1) }}</div>
			<div><Localized label="EoA.Labels.TierCount" :format-args="{ tier: 3 }" />: {{ tier3Talents }}/{{ Math.max(0, tier2Talents - 1) }}</div>
			<div><Localized label="EoA.Labels.TierCount" :format-args="{ tier: 4 }" />: {{ tier4Talents }}/{{ Math.max(0, tier3Talents - 1) }}</div>
			<div><Localized label="EoA.Labels.TierCount" :format-args="{ tier: 5 }" />: {{ tier5Talents }}/{{ Math.max(0, tier4Talents - 1) }}</div>
		</div>
	</section>
</template>
