...
import Characteristic from '../../components/character/Characteristic.vue';
...
const talents = computed(() => actor.value.items.filter((i) => i.type === 'talent') as GameItem<TalentDataModel>[]);
const abilities = computed(() => actor.value.items.filter((i) => i.type === 'ability') as GameItem<AbilityDataModel>[]);
const equipment = computed(() => actor.value.items.filter((i) => EQUIPMENT_TYPES.includes(i.type)) as GameItem<EquipmentDataModel>[]);
const injuries = computed(() => actor.value.items.filter((i) => i.type === 'injury') as GameItem<InjuryDataModel>[]);
...
const rankUpLabel = game.i18n.localize('EoA.Labels.RankUp');
const rankDownLabel = game.i18n.localize('EoA.Labels.RankDown');
...
async function rollAttack(weapon: GameItem) {
...
async function adjustTalentOrSkillRank(item: GameItem<TalentDataModel> | GameItem<SkillDataModel>, adjustment: number) {
...
function skillForWeapon(weapon: GameItem): [name: string, id: string] {
...
function damageForWeapon(weapon: GameItem): number {
...
<template>
	<div class="adversary-sheet">
		...
		<nav class="sheet-tabs" data-group="primary">
			...
			<a class="item" data-tab="stats"><Localized label="EoA.Tabs.Data" /></a>
			<a class="item" data-tab="miscellaneous"><Localized label="EoA.Tabs.Miscellaneous" /></a>
		...
		<section class="sheet-body">
				...
				<section class="stats-tab">
					...
					<div class="characteristics-container">
						<div class="characteristics-row">
							<Characteristic
								label="EoA.Characteristics.Brawn"
							...
							/>
							<Characteristic
								label="EoA.Characteristics.Agility"
							...
							/>
							<Characteristic
								label="EoA.Characteristics.Intellect"
							...
							/>
							<Characteristic
								label="EoA.Characteristics.Cunning"
							...
							/>
							<Characteristic
								label="EoA.Characteristics.Willpower"
							...
							/>
							<Characteristic
								label="EoA.Characteristics.Presence"
					...
					<div class="adversary-items">
						<slot name="skills">
							<span class="header"><Localized label="EoA.Adversary.Skills" />:</span>
							<div class="container">
					...
					<div class="adversary-items">
						<span class="header"><Localized label="EoA.Adversary.Talents" />:</span>
						<div class="container">
							<Localized v-if="talents.length === 0" label="EoA.Adversary.None" />
					...
					<div class="adversary-items">
						<span class="header"><Localized label="EoA.Adversary.Abilities" />:</span>
						<div class="container">
							<Localized v-if="abilities.length === 0" label="EoA.Adversary.None" />
					...
					<div class="adversary-items">
						<span class="header"><Localized label="EoA.Adversary.Equipment" />:</span>
						<div class="container">
							<Localized v-if="equipment.length === 0" label="EoA.Adversary.None" />
							<ContextMenu v-for="item in equipment.filter((i) => i.type === 'weapon')" :key="item.id" class="weapon">
								...
								<a @click="rollAttack(item)">
									...
									(<span class="weapon-details">
										...
										<span>Range [<Localized :label="`EoA.Range.${item.system.range.capitalize()}`" />]</span>
							...
							<ContextMenu v-for="item in equipment.filter((i) => i.type !== 'weapon')" :key="item.id" class="inventory-item" :disable-menu="!context.data.editable">
								...
								<a @click="editItem(item)">
									...
									<span v-if="item.type === 'armor'" style="padding-right: 0.2em"> (+{{ item.system.soak }} <Localized label="EoA.Labels.Soak" />) </span>
			...
			<div class="tab" data-tab="miscellaneous">
				<section class="motivations">
					<div class="adversary-items">
						<span class="header"><Localized label="EoA.Labels.CriticalInjuries" />:</span>
						<div class="container">
							<Localized v-if="injuries.length === 0" label="EoA.Adversary.None" />
					...
					<div class="header"><Localized label="EoA.Labels.Motivations" /></div>
					<div class="container">
						<!-- Strength -->
						<div class="motivation">
							<div class="header">
								<span><Localized label="EoA.Labels.Strength" /></span>
						...
						<div class="motivation">
							<div class="header">
								<span><Localized label="EoA.Labels.Flaw" /></span>
						...
						<div class="motivation">
							<div class="header">
								<span><Localized label="EoA.Labels.Desire" /></span>
						...
						<div class="motivation">
							<div class="header">
								<span><Localized label="EoA.Labels.Fear" /></span>
